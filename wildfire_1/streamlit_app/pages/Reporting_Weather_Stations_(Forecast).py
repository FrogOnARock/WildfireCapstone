...
from api_client import get_reporting_weather_forecast_stations_dates, get_reporting_weather_forecast_stations_by_date
...
st.title("Reporting Weather Stations (Forecast)")
...
query_type = st.radio("Choose a query", [
    "Get Reporting Weather Stations (Forecast) by Date"
])

# Query form logic
params = {}
if query_type == "Get Reporting Weather Stations (Forecast) by Date":
    st.markdown("### Parameters for Get Reporting Weather Stations (Forecast) by Date")
    rep_dates = [f["rep_date"] for f in get_reporting_weather_forecast_stations_dates() if "rep_date" in f]
...
if st.button("Run Query"):
    if query_type == "Get Reporting Weather Stations (Forecast) by Date":
        data = get_reporting_weather_forecast_stations_by_date(str(params["date"]))
    else:
        data = []

    st.session_state.fire_data_rwsf = data

if st.button("Clear Results"):
    st.session_state.fire_data_rwsf = None

# Show results
data = st.session_state.get("fire_data_rwsf", None)
if not data:
    st.stop()
...
#collect the data in GeoJSON Format
geojson = {
    ...
    "config": {
        "visState": {
            ...
            "interactionConfig": {
                "tooltip": {
                    "fieldsToShow": {
                        "Reporting Weather Stations (Forecast)": [
...
kepler_map = KeplerGl(data={"Reporting Weather Stations (Forecast)": geojson})
...
st.markdown("---")
st.subheader("Reporting Weather Station (Forecast) Observations")
